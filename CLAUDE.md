...
- `CharacterInitializer` handles LLM invocation and JSON file persistence to `data/characters/`
...
- **Output**: Events saved as `data/events/{uuid}.json` with transcript, summary, outcome
...
API uses Pydantic request/response models for validation. Characters and events are persisted as JSON files in `data/` directory, not a database.
...
- `data/characters/{uuid}.json`: Generated character data
- `data/events/{uuid}.json`: Event transcripts with metadata
...
- **Python**: Follows Ruff configuration in `pyproject.toml` (line length 88, strict type checking with mypy)
